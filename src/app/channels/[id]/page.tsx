...
import {
  ArrowLeft, Users, Bus, UserCheck, Briefcase, Route, DollarSign,
...
export default function ChannelDetailPage() {
  ...
  const fetchChannel = useCallback(() => {
    if (params.id) {
      apiClient.get(`/super-admin/channels/${params.id}`)
  ...
  useEffect(() => {
    if (activeTab === 'analytics' && params.id && !analyticsData && !analyticsLoading) {
      setAnalyticsLoading(true);
      apiClient.get(`/super-admin/channels/${params.id}/analytics`)
  ...
  }, [activeTab, params.id, analyticsData, analyticsLoading]);
  ...
  const handleAction = async (action: string) => {
    ...
    try {
      if (action === 'suspend') {
        ...
        await apiClient.patch(`/super-admin/channels/${params.id}/suspend`, { reason });
        toast.success('Channel suspended successfully');
      } else if (action === 'activate') {
        await apiClient.patch(`/super-admin/channels/${params.id}/activate`);
        toast.success('Channel activated successfully');
      }
      // Refresh channel data
      const res = await apiClient.get(`/super-admin/channels/${params.id}`);
  ...
  const handleSaveEdit = async () => {
    setSaving(true);
    try {
      await apiClient.put(`/super-admin/channels/${params.id}`, editForm);
  ...
  const overviewContent = (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="font-semibold mb-4 dark:text-gray-100">Channel Information</h3>
                ...
                <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all ${
                      (card.count / card.quota) >= 0.9
                        ? 'bg-red-500'
                        : (card.count / card.quota) >= 0.7
                          ? 'bg-amber-500'
                          : 'bg-indigo-500'
                    }`}
        ...
        <div className="space-y-4">
          <div>
            <div className="flex items-center justify-between text-sm mb-2">
              <span className="text-gray-600 flex items-center gap-2"><Bus size="14" /> Buses</span>
              <span className="font-medium">{rc.buses || 0} / {maxBuses}</span>
            </div>
            <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-3">
              <div
                className={`h-3 rounded-full ${
                  (rc.buses || 0) / maxBuses >= 0.9 ? 'bg-red-500' : (rc.buses || 0) / maxBuses >= 0.7 ? 'bg-amber-500' : 'bg-blue-500'
                }`}
                style={{ width: `${Math.min(((rc.buses || 0) / maxBuses) * 100, 100)}%` }}
              />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between text-sm mb-2">
              <span className="text-gray-600 flex items-center gap-2"><Activity size="14" /> Trips / Day</span>
              <span className="font-medium">{rc.tripsToday || 0} / {maxTripsPerDay}</span>
            </div>
            <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-3">
              <div
                className={`h-3 rounded-full ${
                  (rc.tripsToday || 0) / maxTripsPerDay >= 0.9 ? 'bg-red-500' : (rc.tripsToday || 0) / maxTripsPerDay >= 0.7 ? 'bg-amber-500' : 'bg-indigo-500'
                }`}
                style={{ width: `${Math.min(((rc.tripsToday || 0) / maxTripsPerDay) * 100, 100)}%` }}
              />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between text-sm mb-2">
              <span className="text-gray-600 flex items-center gap-2"><Briefcase size="14" /> Staff</span>
              <span className="font-medium">{rc.staff || 0} / {maxStaff}</span>
            </div>
            <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-3">
              <div
                className={`h-3 rounded-full ${
                  (rc.staff || 0) / maxStaff >= 0.9 ? 'bg-red-500' : (rc.staff || 0) / maxStaff >= 0.7 ? 'bg-amber-500' : 'bg-purple-500'
                }`}
                style={{ width: `${Math.min(((rc.staff || 0) / maxStaff) * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>
  ...
  // ─── Financials Tab ─────────────────────────────────────────────
  const revenueByMonth = fs.revenueByMonth || [];
  const paymentBreakdown = fs.paymentBreakdown || [];
  ...
  const financialsContent = (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          title="Total Revenue"
          value={`Rs. ${(fs.totalRevenue || 0).toLocaleString()}`}
          icon={<DollarSign size="20" />}
          color="green"
          change={fs.revenueGrowth}
          changeLabel="vs last period"
        />
        <KPICard
          title="Today's Revenue"
          value={`Rs. ${(fs.todayRevenue || 0).toLocaleString()}`}
          icon={<TrendingUp size="20" />}
          color="blue"
        />
        <KPICard
          title="This Week"
          value={`Rs. ${(fs.weekRevenue || 0).toLocaleString()}`}
          icon={<CalendarDays size="20" />}
          color="indigo"
        />
        <KPICard
          title="This Month"
          value={`Rs. ${(fs.monthRevenue || 0).toLocaleString()}`}
        ...
        <ChartCard title="Payment Method Breakdown" subtitle="Revenue by payment type">
          {paymentBreakdown.length > 0 ? (
            <div className="space-y-4 pt-2">
              {paymentBreakdown.map((pm: any, index: number) => {
                const total = paymentBreakdown.reduce((sum: number, p: any) => sum + (p.amount || p.revenue || 0), 0);
  ...
  const tripTrends = an.tripTrends || [];
  const growthMetrics = an.growthMetrics || [];
      ...
      {analyticsLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <KPICard
              title="Total Trips"
              value={an.totalTrips || 0}
              icon={<Activity size="20" />}
              color="blue"
              change={an.tripsGrowth}
              changeLabel="vs last period"
            />
            <KPICard
              title="Completion Rate"
              value={`${an.completionRate || 0}%`}
              icon={<CheckCircle2 size="20" />}
              color="green"
            />
            <KPICard
              title="Avg Occupancy"
              value={`${an.avgOccupancy || 0}%`}
              icon={<Users size="20" />}
              color="indigo"
            />
            <KPICard
              title="Total Bookings"
              value={an.totalBookings || 0}
              icon={<BarChart3 size="20" />}
              color="purple"
              change={an.bookingsGrowth}
              changeLabel="vs last period"
            />
          </div>
          ...
          <ChartCard title="Trip Trends" subtitle="Daily trip volume">
            {tripTrends.length > 0 ? (
              ...
              <div className="h-[300px] flex items-center justify-center text-gray-400 text-sm">
                No trip trend data available
              </div>
            )}
          </ChartCard>

          {/* Channel Growth Metrics */}
          <ChartCard title="Channel Growth" subtitle="Growth over time">
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/channels')} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg dark:text-gray-400">
            <ArrowLeft size="20" />
          </button>
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{channel.companyName}</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Channel details and management</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            {canSuspend && (channel.status === 'ACTIVE' ? (
              <button
                onClick={() => handleAction('suspend')}
                disabled={actionLoading}
                className="flex items-center gap-2 px-3 py-1.5 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg text-xs font-medium hover:bg-red-100 dark:hover:bg-red-900/30 disabled:opacity-50 transition-colors"
              >
                <Ban size="14" /> Suspend
              </button>
            ) : (
              <button
                onClick={() => handleAction('activate')}
                disabled={actionLoading}
                className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-lg text-xs font-medium hover:bg-emerald-100 dark:hover:bg-emerald-900/30 disabled:opacity-50 transition-colors"
              >
